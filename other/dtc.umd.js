    ...
    */
    ...
    function (_THREE$Object3D) {
      ...
      _createClass(Area, [{
        ...
        value: function getLine(coords, pros) {
          ...
          coords.forEach(function (coord) {
            ...
            for (var i = 0, l = pts.length; i < l; i++) {
              line.vertices.push(new THREE.Vector3(pts[i].x, pts[i].y, _this3.userData.extrude.amount + _this3.userData.extrude.amount / 100));
            }
        ...
        value: function getGeoMesh(geo, pros) {
          ...
          if (this.userData.hasPhong) geoMesh = new THREE.Mesh(geo, new THREE.MeshPhongMaterial(mateOption));else geoMesh = new THREE.Mesh(geo, new THREE.MeshLambertMaterial(mateOption)); //var geoMesh = THREE.SceneUtils.createMultiMaterialObject(geo,[new THREE.MeshPhongMaterial(mateOption),new THREE.MeshBasicMaterial({wireframe:true,color:0xffffff,transparent:true,opacity:0.35})])
          return geoMesh;
        }
        /**
         * Area的网格对象
        ...
        value: function onmouseout(dispatcher, event) {
          ...
          .start();
          new TWEEN.Tween(this.mesh.material.color).to(new THREE.Color(colorToHex(this.userData.color)), this.userData.hoverAnimaTime) //.easing(TWEEN.Easing.Quartic.Out)
          .start();
        ...
        value: function onmouseover(dispatcher, event) {
          ...
          .start(); //区域移入颜色
          new TWEEN.Tween(this.mesh.material.color).to(new THREE.Color(colorToHex(this.userData.hoverColor)), this.userData.hoverAnimaTime) //.easing(TWEEN.Easing.Quartic.Out)
          .start();
          dispatcher.dispatchEvent({
            type: 'mouseover',
            target: this,
    ...
    function (_THREE$Object3D) {
      ...
      function DataRange(pros) {
        ...
        var boxMate = new THREE.MeshPhongMaterial({
          color: pros.color
        });
    ...
    function (_THREE$Sprite) {
      ...
      function Mark(pros) {
        ...
        _this.material = new THREE.SpriteMaterial({
          map: Mark.texture,
          color: pros.color,
          blending: THREE.AdditiveBlending
        });
    ...
    function (_THREE$Line) {
      ...
      function Line(pros) {
        ...
        var v3s = [new THREE.Vector3(x1, y1, pros.extrudeHeight), new THREE.Vector3((x1 + x2) / 2, (y1 + y2) / 2, pros.extrudeHeight + pros.spaceHeight), new THREE.Vector3(x2, y2, pros.extrudeHeight)]; //画弧线
        var curve = _construct(THREE.QuadraticBezierCurve3, v3s);
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Line).call(this, geometry, material));
        Object.assign(_this.userData, pros); //线条光晕效果
        if (pros.hasHalo) {
          _this.initHalo(geometry);
        } //当前线条索引
        _this.index = Line.count++;
        Line.array.push(_assertThisInitialized(_assertThisInitialized(_this)));
        return _this;
      }
      /**
       * 初始化发光线
      ...
      _createClass(Line, [{
        ...
        value: function initHalo(geometry) {
          ...
          shader.uniforms = {
            ...
            color: {
              value: new THREE.Color(colorToHex(this.userData.haloColor))
            },
            texture: {
              value: Line.texture
            }
        ...
        value: function setColor(color, haloColor) {
          //基线
          if (typeof color !== 'undefined') this.material.color = new THREE.Color(colorToHex(color)); // //光线
